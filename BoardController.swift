...
class BoardController: NSObject,
  ...
  func resetBoardWithCurrentSettings() {
      applyNumLettersSettings(with: [:])  // Reset the board with the current settings
      applyNumGuessesSettings(with: [:])
      applyThemeSettings(with: [:])
      applyIsAlienWordleSettings(with: [:])
      numTimesGuessed = 0
      collectionView.reloadData()
  }
